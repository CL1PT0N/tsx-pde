...
export default function InfoSection({lecturer,title,startdate,enddate,tag}:infoType) {
  return (
    <Flex w="50%"  gap={4} flexDir='column' justifyContent='space-between' alignItems='start' p={2}>
      <Box>
        <Text noOfLines={1} color="grey" fontWeight={400} as="h6" transform='capitalize'>
          {lecturer}
        </Text>
      ...
      <Box>{isoToString(startdate)} - {isoToString(enddate)}</Box>
    </Flex>
  );
}
